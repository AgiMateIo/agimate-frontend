...
export default function ChannelConfigForm({
  ...
  // App tools come as {name, description}; integration tools as full ConnectorToolSpec —
  // the dropdown only needs the common subset.
  const [tools, setTools] = useState<Array<Pick<ConnectorToolSpec, 'name' | 'description'>>>([]);
  const [toolSpecs, setToolSpecs] = useState<Record<string, ConnectorToolSpec>>({});
  ...
  useEffect(() => {
    ...
    const specFetcher = apiService
      ...
      .catch(() => ({} as Record<string, ConnectorToolSpec>));
  ...
  useEffect(() => {
    ...
    if (!spec) return;
    const next = buildToolParamsTemplate(spec.inputSchema, {
      includeOptional,
  ...
  const selectedSpecHasOptional = useMemo(() => {
    ...
    const spec = toolSpecs[replyToolName];
    if (!spec || !spec.inputSchema.properties) return false;
    const required = new Set(spec.inputSchema.required ?? []);
    return Object.keys(spec.inputSchema.properties).some((k) => !required.has(k));
  ...
  const resetParamsToDefault = useCallback(() => {
    ...
    if (spec) {
      setParamsText(
        buildToolParamsTemplate(spec.inputSchema, { includeOptional, sample: capturedSample }),
      );
    } else {
