...
export default function DesktopPage() {
  ...
  return (
    <div className="min-h-screen text-foreground">
      ...
      <header className="sticky top-0 z-50 border-b border-border/40 backdrop-blur-md bg-background/80">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <nav className="flex items-center gap-6 text-sm text-muted">
            ...
            <a href="#plugins" className="hidden sm:inline hover:text-foreground transition-colors">
              {t('nav.plugins')}
            </a>
            <a href="#specs" className="hidden sm:inline hover:text-foreground transition-colors">
              {t('nav.specs')}
            </a>
      ...
      {/* Plugin System */}
      <section id="plugins" className="mx-auto max-w-6xl px-6 py-20">
        <h2 className="mb-4 text-center text-3xl font-bold tracking-tight">{t('plugins.title')}</h2>
        <p className="mx-auto mb-14 max-w-xl text-center text-muted">{t('plugins.subtitle')}</p>

        {/* Two plugin type cards */}
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded-2xl border border-border/50 border-t-4 border-t-warning bg-surface p-6 transition-colors hover:border-accent/30">
          ...
          <div className="rounded-2xl border border-border/50 border-t-4 border-t-accent bg-surface p-6 transition-colors hover:border-accent/30">
            <div className="mb-4 flex items-center gap-3">
        ...
        {/* Comparison table */}
        <div className="mt-10 overflow-x-auto rounded-2xl border border-border/50 bg-surface">
      ...
      {/* Technical Specs */}
      <section id="specs" className="mx-auto max-w-6xl px-6 py-20">
        <h2 className="mb-4 text-center text-3xl font-bold tracking-tight">{t('specs.title')}</h2>
        <p className="mx-auto mb-14 max-w-xl text-center text-muted">{t('specs.subtitle')}</p>
        <div className="mx-auto max-w-2xl overflow-x-auto rounded-2xl border border-border/50 bg-surface">
          <table className="w-full text-left text-sm">
            <tbody>
              {specRows.map((row) => (
                <tr key={row.param} className="border-b border-border/30 last:border-0">
                  <td className="px-6 py-3 font-medium">{row.param}</td>
                  <td className="px-6 py-3 text-muted">{row.value}</td>
                </tr>
              ))}
            </tbody>
          </table>
                ...
                {i === 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-surface-secondary transition-colors"
                    >
      ...
      <footer className="border-t border-border/40">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-6 py-8 sm:flex-row">
          ...
          <div className="flex items-center gap-6 text-sm text-muted">
            <a href="https://github.com/AgiMateIo/desktop" target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">{t('footer.github')}</a>
...
function FeatureCard({
  ...
  return (
    <div className={`rounded-2xl border border-border/50 border-l-4 ${borderColor} bg-surface p-6 transition-colors hover:border-accent/30 ${hoverBorder}`}>
