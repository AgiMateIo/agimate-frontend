...
- **Channels**: CRUD `/channels/`. A channel's conversations are not its own resource — they are `/sessions/` filtered by `channelId` (see **Sessions** below).
...
- **Sessions** (a conversation with an agent, whatever carries it — webchat, a messenger, an IDE): one resource `/manage/sessions/` for the listing, the history, the read pointer, the rename and the close. Filters `agentId` / `channelId` / `connectorCode`, all optional and ANDed, freshest activity first. **Without a filter this is every conversation the user has**, not just their chats — a connection's channel-less event stream (`channelId: null`) and every messenger thread are in there, so a chat list means `connectorCode=webchat` and one agent's conversations across all channels mean `agentId` alone. The row: `id` (**not** `sessionId`), `agentId`, `channelId`, `connectorCode`, `title`, `lastActivityAt` (**not** `lastMessageAt`), `closedAt`, `createdAt`, plus three fields counted off the webchat log alone and therefore `0`/`null`/`false` outside it — `unreadCount` (AGENT `answer`/`error` past the read pointer; `progress` and own messages never count), `lastMessage` (preview cut to 160 chars server-side; `text: null` + `hasAttachments` = an attachment-only message, so never render the empty string) and `isRunning` (a run executing or queued — for *restoring* the "working…" indicator when a screen opens; it goes out live with the answer/error event, and a queued run stops counting after 15 min). **Never badge a row of another connector**: what is unread in a messenger is that messenger's business. The same row shape answers every call — listing, `GET /sessions/{id}`, rename, close and the webchat start — so a response goes straight back into the list. History `GET /sessions/{id}/messages/` is one shape for every channel, newest-first (reverse a page to read a transcript): `id`, `messageId`, `direction` (`USER`/`AGENT`), `stream`, `text`, `parts`, `createdAt` — where **`messageId` and `parts` are null outside webchat** (no delivery id, no attachments in this shape), so dedupe non-webchat history by `id`. Rename `PATCH /sessions/{id} {title}` — 80 characters, over it a 400 rather than a silent trim, and it overrides the title derived from the first message for good (no way back to the automatic one). Close `POST /sessions/{id}/close` (soft: history stays readable, sending returns 400). Read pointer `POST /sessions/{id}/read`, body optional — **`lastReadMessageId` is a history row's `id`, not its `messageId`** (that one is the delivery key and carries no order; sending it is a 400). The pointer only moves forward, so a stale call is a no-op, and sending a message or closing the session clears the badge server-side.
- **Webchat** (the dashboard chat's transport, and only that): `POST /webchat/sessions` starts a chat with an agent (`{agentId}`, answers a **session row** — `id`, not `sessionId`), `POST /webchat/sessions/{id}/messages` sends (attachments are `parts: [{fileId}]`, up to 5, uploaded beforehand through `POST /manage/files` — the key is `fileId` even though the file row calls it `id`; a file failing ownership/readiness/expiry is a 400, never a silent drop), `POST /webchat/sessions/{id}/token` mints the per-session Centrifugo token; agent replies arrive as `webchat_message` events (streams: progress/answer/error, at-least-once — dedupe by `messageId`). `GET /webchat/contacts/` (agents as chat contacts) exists but nothing in the dashboard uses it — chat is per-agent, there is no contacts screen. **The old `/webchat/sessions/…` listing, history, read and DELETE are gone** — 404 now; they are `/manage/sessions/…` above.
...
`apiService` (default export of `src/services/api.ts`) is a singleton exposing all backend calls behind a **flat facade**. `api.ts` only composes per-domain modules from `src/services/modules/` (`admin`, `agents`, `apps`, `skills`, `llmProviders`, `channels`, `chatSessions`, `boards`, `connections`, `connectors`, `files`, `agenticTeams`, `logs`, `misc`, `webchat`) over the shared transport core in `src/services/httpClient.ts`. Consumers call `apiService.getAgent(...)` etc. and never touch the modules directly. When adding an endpoint, put the method in the matching domain module (or add a new module and spread it into the facade in `api.ts`).
...
│                                  #   centrifugo, chat-sessions, tool-use-logs,
│                                  #   connectors via skills.ts) + index.ts
...
Types live in `src/types/`, one file per domain (`agents`, `skills`, `apps`, `channels`, `boards`, `agentic-teams`, `connections`, `llm-providers`, `agent-connections`, `agent-skills`, `connector-jobs`, `webhooks`, `centrifugo`, `chat-sessions`, `tool-use-logs`, `files`, `admin`, …), re-exported from `index.ts`.
```typescript
